...
//					setTitleName(toChatUsername,nickNameTV);
					new GetUserInfoByNameTask().execute(toChatUsername,nickNameTV,1);
				}
			}
...
//	}
	...
	class GetUserInfoByNameTask extends AsyncTask<Object, Void, Integer>{
		...
		private int actionType;//作用1：设置聊天对象获取昵称
								//作用2：添加好友时获取信息
		@Override
		...
		protected void onPostExecute(Integer result) {
			...
			case Global.code.SUCCESS: {
				if(mUser != null){
					if(actionType == 1){//设置聊天对象
						nameTV.setText(mUser.getNickName());
						new LXHUserDao(App.applicationContext).insertUser(mUser);
					}else if(actionType == 2){//添加好友时获取用户信息
						new Thread(new Runnable() {
							public void run() {
								try {
									//demo写死了个reason，实际应该让用户手动填入
									EMContactManager.getInstance().addContact(mUser.getuName(), "加个好友呗");
									//将添加的好友持久到本地数据库
									addToLocalDB(mUser.getuName(),mUser.getNickName(),mUser.getHeadUrl());
									runOnUiThread(new Runnable() {
										public void run() {
											Toast.makeText(ChatActivity.this, "成功添加"+mUser.getNickName(), Toast.LENGTH_SHORT).show();
										}
									});
...
//		 body.setSendOriginalImage(true);
	...
	public void addFriend(View view) {
		new GetUserInfoByNameTask().execute(toChatUsername,null,2);
	}
